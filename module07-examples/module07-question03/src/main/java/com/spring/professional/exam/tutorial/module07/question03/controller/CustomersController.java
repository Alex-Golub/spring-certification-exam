...
public class CustomersController {
  ...
  public Resources<Resource<Customer>> listCustomers() {
    List<Resource<Customer>> customers = StreamSupport
            .stream(customersDao.findAll().spliterator(), false)
            .map(customer -> new Resource<>(customer, linkTo(methodOn(CustomersController.class).getCustomer(customer.getId())).withSelfRel()))
            .collect(Collectors.toList());

    return new Resources<>(customers, linkTo(methodOn(IndexController.class).index()).withRel("index"));
  }
  ...
  public Resource<Customer> createCustomer(@RequestBody @Valid Customer customer) {
    ...
    return new Resource<>(createdCustomer, linkTo(methodOn(CustomersController.class).getCustomer(customer.getId())).withSelfRel());
  }
  ...
  public Resources<Resource<Customer>> updateCustomers(@RequestBody @Valid Collection<Customer> customers) {
    ...
    List<Resource<Customer>> customerResources = StreamSupport
            .stream(updatedCustomers.spliterator(), false)
            .map(customer -> new Resource<>(customer, linkTo(methodOn(CustomersController.class).getCustomer(customer.getId())).withSelfRel()))
            .collect(Collectors.toList());

    return new Resources<>(customerResources, linkTo(methodOn(CustomersController.class).listCustomers()).withRel("customers"));
  }
