...
public class Example08RequestAttribute {
  ...
  private static final String PROCESSING_TIME = "processing time = [%s]%n";
  ...
  public String example08A(@RequestAttribute("processing-time") String processingTime) {
    return String.format(PROCESSING_TIME, processingTime);
  }
  ...
  @ResponseBody
  public String example08C(@RequestAttribute(value = "non-existing-processing-time", required = false)
  ...
  @ResponseBody
  public String example08D(@RequestAttribute(value = "non-existing-processing-time", required = false)
                                   Optional<LocalDateTime> processingTime) {
    return String.format(PROCESSING_TIME, processingTime.orElse(LocalDateTime.MIN));
  }
}
