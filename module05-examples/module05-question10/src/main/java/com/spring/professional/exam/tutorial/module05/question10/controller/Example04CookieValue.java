...
public class Example04CookieValue {
  ...
  private static final String USER_ID_FROM_COOKIE = "user-id from cookie = [%s]%n";
  ...
  public String example04ReadCookie1(@CookieValue("user-id") String userId) {
    return String.format(USER_ID_FROM_COOKIE, userId);
  }
  ...
  public String example04ReadCookie2(@CookieValue("user-id") Cookie cookie) {
    return String.format(
            "Received cookie name = [%s], value = [%s]%n",
  ...
  @ResponseBody
  public String example04ReadCookie4(@CookieValue(value = "user-id", required = false, defaultValue = "N/A")
                                             String userId) {
    return String.format(USER_ID_FROM_COOKIE, userId);
  }
  ...
  public String example04ReadCookie5(@CookieValue(value = "user-id") Optional<String> userId) {
    return String.format(USER_ID_FROM_COOKIE, userId.orElse("N/A"));
  }
}
