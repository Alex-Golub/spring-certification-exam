...
public class Example03MatrixVariable {
  ...
  @ResponseBody
  public String example03A(@MatrixVariable int id) {
    return String.format("Received request id = [%d]%n", id);
  }

  // curl "http://localhost:8080/example03B/employees/id=1;name=John"
  //   Received request id = [1], name = [John]
  @GetMapping("/example03B/employees/{employee}")
  @ResponseBody
  public String example03B(@MatrixVariable int id,
                           @MatrixVariable String name) {
    return String.format("Received request id = [%d], name = [%s]%n", id, name);
  }

  // curl "http://localhost:8080/example03C/employees/id=1;name=John;department=HR/city/code=NYC;location=US"
  //   Received request department = [HR], cityMatrixVariables = [US]
  @GetMapping("/example03C/employees/{employee}/city/{city}")
  @ResponseBody
  public String example03C(@MatrixVariable(pathVar = "employee", name = "department") String department,
                           @MatrixVariable(pathVar = "city", name = "location") String location) {
    return String.format("Received request department = [%s], cityMatrixVariables = [%s]%n",
                         department, location);
  }

  // curl "http://localhost:8080/example03D/employees/id=1;name=John;city=NYC"
  //   Received request matrixVariables = [{id=1, name=John, city=NYC}]
  @GetMapping("/example03D/employees/{employee}")
  @ResponseBody
  public String example03D(@MatrixVariable Map<String, String> matrixVariables) {
    return String.format("Received request matrixVariables = [%s]%n", matrixVariables);
  }

  // curl "http://localhost:8080/example03E/employees/id=1;name=John;department=HR/city/code=NYC;location=US"
  //   Received request matrixVariables = [{id=1, name=John, department=HR, code=NYC, location=US}]
  ...
  //   Received request
  //   employeeMatrixVariables = [{id=1, name=John, department=HR}],
  //   cityMatrixVariables = [{code=NYC, location=US}]
  @GetMapping("/example03F/employees/{employee}/city/{city}")
  @ResponseBody
  public String example03F(@MatrixVariable(pathVar = "employee") Map<String, String> employeeMatrixVariables,
                           @MatrixVariable(pathVar = "city") Map<String, String> cityMatrixVariables) {
    return String.format("Received request employeeMatrixVariables = [%s], cityMatrixVariables = [%s]%n",
